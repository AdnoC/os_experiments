...
pub fn init() {
    ...
    let res = mbox.send_and_poll_recieve_one(FBAllocateBufferRequest { alignment: 16}).unwrap().unwrap();
    println!("Res: {:?}", res);
    ...
    // println!("Got the mailbox");
    // let res = mbox.send_and_poll_recieve_one(FBSetPhysicalSizeRequest { width: 640, height: 480 }).unwrap();
    // println!("Res: {:?}", res);
    // let res = mbox.send_and_poll_recieve_one(FBSetVirtualSizeRequest { width: 640, height: 480 }).unwrap();
    // println!("Res: {:?}", res);
    // let res = mbox.send_and_poll_recieve_one(FBSetBitsPerPixelRequest { bpp: core::mem::size_of::<Pixel>() as u32 * 8}).unwrap();
    // println!("Res: {:?}", res);
    // let res = mbox.send_and_poll_recieve_one(FBAllocateBufferRequest { alignment: 16}).unwrap().unwrap();
    // println!("Res: {:?}", res);
    //
